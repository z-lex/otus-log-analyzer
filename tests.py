...
from log_analyzer import (
    ...
    LogfileLineInfo,
    LogfileType,
    get_last_logfile_info,
    get_logfile_stats,
    parse_logfile,
    parse_logfile_line,
    render_template,
    report_filename_from_date,
    select_last_logfile,
)
...
class SelectLastLogfileTest(BaseLogAnalyzerTestCase):
    ...
    def setUp(self) -> None:
        ...
        self.file_info_expected = LogfileInfo(
...
class GetLastLogfileTest(BaseLogAnalyzerTestCase):
    ...
    def test(self):
        ...
        self.assertIsInstance(result, LogfileInfo, "get_last_logfile: wrong return type")
        self.assertEqual(
            self.last_logfile_expected, result, "get_last_logfile: wrong result value"
        )
...
class CreateReportForFileTest(BaseLogAnalyzerTestCase):
    ...
    def setUp(self):
        ...
        self.new_report_path = Path(self.config["REPORT_DIR"]).joinpath(
            report_filename_from_date(date=self.logfile_to_parse.date)
        )
    ...
    def test(self):
        table_json = [
            ...
            for stats in get_logfile_stats(
                logfile_info=self.logfile_to_parse,
